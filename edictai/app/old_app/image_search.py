...
def merged(data):
    ...
    #                 break
        ...
        images_dir = "D:\EDI\edictAI\edictAI\edict_ai\images"

        # Loop through folders named 1, 2, 3, etc. inside the "images" folder
        for i in range(1,2):
            folder_name = str(i)
            ...
            if os.path.exists(folder_path):
                # Get the filename of the image inside the folder
                file_list = os.listdir(folder_path)
                if len(file_list) == 1:
                    old_name = os.path.join(folder_path, file_list[0])

                    # Define the new filename
                    new_name = os.path.join(folder_path, f"chunk_{meraNum}")

                    # Rename the image
                    os.rename(old_name, new_name)

        
            image_filename = f"images/{meraNum}/chunk_{meraNum}"
    ...
    # API website to get your (free) key: https://rapidapi.com/k_1/api/large-text-to-speech/
        ...
        sf.write(f"chunk_{meraNum}.wav", speech.numpy(), samplerate=16000)
